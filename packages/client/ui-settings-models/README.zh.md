...
Models 页面还通过 `openaiCodex` Remote 渲染 OpenAI 会员卡片。它打开设备授权页，展示用户码和等待状态，完成后显示可获得的 Codex 套餐、五小时／每周额度、附加额度与 credits。OAuth 凭据不会进入浏览器状态；跨 Remote 边界的只有账户状态和有界用量投影。
