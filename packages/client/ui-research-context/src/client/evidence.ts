...
/** Client-only projection of the same lightweight support-navigation rules. */
export function evaluateIdeaSupport(inquiry: ResearchInquiryState | undefined): ResearchEvidenceSupportProjection {
  const requirements = inquiry?.nodes.filter(node => node.kind === 'evidence-requirement') ?? []
  if (requirements.length === 0) {
    return Object.freeze({ status: 'untracked', requirements: [], candidateCount: 0, challengedCount: 0 })
  }
  ...
  const assessments: ResearchEvidenceRequirementAssessment[] = requirements.map((requirement) => {
    ...
    return Object.freeze({
      ...
      status: challenging.length > 0 ? 'challenged' as const : supporting.length > 0 ? 'candidate' as const : 'open' as const,
  ...
  const status = challengedCount > 0 ? 'challenged' : candidateCount > 0 ? 'candidate' : 'open'
