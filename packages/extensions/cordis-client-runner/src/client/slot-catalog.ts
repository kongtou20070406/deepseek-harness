...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.context.details',
    kind: 'list',
    scope: 'session',
    summary: 'Optional plugin-owned detail sections inside the existing ContextMeter panel.',
    doc: 'Optional plugin-owned detail sections inside the existing ContextMeter\npanel. Entries receive the normal session standard kit and render only\ntheir own rows; ui-conversation owns panel placement and spacing.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [
      'client-ui-research-context ResearchContextDetails id \'research-context\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.context.details\', () => ctx.slots.register(\n      { name: \'conversation.context.details\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-research-context IdeaDock id \'research-idea\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-models CodexUsageMeter id \'codex-usage\'',
    ],
    replaceRisk: 'none',
