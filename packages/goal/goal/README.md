...
Activation is never persisted. A fresh cache starts disarmed, and startup/fork edges leave replayed goals inert. A resume edge rearms only a durable active goal, so an interrupted objective continues after a service restart without writing a synthetic goal revision. Paused, blocked, and complete goals remain disarmed. A continuation driver also calls `disarm()` before unload or after durability uncertainty; without a later session resume, an explicit resume mutation is required to arm continuation.
